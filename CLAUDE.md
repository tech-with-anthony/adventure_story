...
## Current Status (as of the fourth session)

**Branch: `claude/debian-gaming-os-skpbqx`** — pushed and up to date at commit `5c29650`.

| | |
|---|---|
| `main` | clean. Contains the game only. |
| Current branch | `claude/debian-gaming-os-skpbqx`, one commit ahead of `main` (`5c29650`) |
| Open PR? | **No.** `5c29650` is pushed but **not** in a pull request yet. |
| Merged this session | PR #3 (added `arcadia-os/`), PR #4 (removed it again) |

**Pick-up point:** open a PR for `5c29650`, or keep committing to that branch.
Note the branch name is a leftover from the Arcadia OS work and no longer
describes its contents — renaming it is reasonable.
...
- **Arcadia OS built, then moved out** — a Debian-based gaming Linux distribution was built in `arcadia-os/`, merged to `main` via PR #3, then removed via PR #4. It now lives in its own repository, **`tech-with-anthony/arcadia-os`**. It shared nothing with this game, so keeping it here muddled the repo's purpose. See the "Arcadia OS" section below before resuming that work — **the ISO has never actually been built.**
...
- **`adventure_standalone.html` regenerated** — it was **stale**, still carrying all four story bugs above. Since this is the file that gets shared (Google Drive), it was shipping the old story. Regenerated by inlining `story-data.js` and `engine.js` into `adventure.html`, and the service-worker registration was dropped from it — a `file://` single-file build can never register one, so it only threw a console error. Verified by playing the standalone through the same Cleric path.
...
4. **Open a PR for `5c29650`** if you want the review trail; the commit is pushed but unreferenced by any PR.

### Cloud-session limits (learned the hard way)

A Claude Code cloud session's **repository allowlist is fixed when the container
is created** and only the `add_repo` tool can extend it. In the fourth session
that could not be approved, so:

- `create_repository` returns **403 Resource not accessible by integration** —
  the GitHub App token cannot create repos at all.
- The git proxy authorises per-repo: `adventure_story` resolves, any other repo
  gets `could not read Password`.
- GitHub MCP calls return `Access denied: repository … is not configured for
  this session`.

None of this is fixable from inside the session, and changing local permission
settings does not reach it. **If a session needs to touch a second repo, attach
that repo when creating the session.** Otherwise hand the user a git bundle
(`git bundle create x.bundle --all`) and let them push — that preserves file
modes, which the GitHub API route does not.
...
## Verifying story changes (fast, no browser)

`story-data.js` is pure data and never touches the DOM, so scene text can be
rendered directly in Node. This is the quickest way to prove a flag-conditional
paragraph actually behaves — far faster than driving a browser, and it isolates
the story from the engine:

```js
global.window = {};
require("./story-data.js");
const S = window.STORY.scenes;

// Render a scene the way engine.js does: strings pass through, fn(state) is called.
const render = (id, flags, charClass) =>
  S[id].paragraphs.map(p => typeof p === "function"
    ? p({ name: "Tessa", charClass: charClass || "cleric", flags: flags || {} })
    : p).join(" ");

// Assert both sides of every conditional you touch:
render("throne_battle_cleric", {});                      // must NOT claim the altar was restored
render("throne_battle_cleric", { altar_restored: true }); // must mention the blessing
```

Two checks worth re-running after any story edit:

```bash
node --check story-data.js          # syntax
# every referenced scene id exists (catches typos in `next`)
node -e 'global.window={};require("./story-data.js");
const S=window.STORY.scenes,ids=new Set(Object.keys(S));let bad=[];
const chk=(n,f)=>{if(typeof n==="string"&&!ids.has(n))bad.push(f+" -> "+n)};
for(const[id,sc]of Object.entries(S))for(const c of(sc.choices||[])){
 if(typeof c.next==="string")chk(c.next,id);
 else if(c.next&&typeof c.next==="object")Object.values(c.next).forEach(v=>chk(v,id));}
console.log(bad.length?"BROKEN:\n"+bad.join("\n"):"ok, "+ids.size+" scenes");'
```

### Auditing flags

The single most useful health check on this story — it finds choices that
promise consequences and deliver none:

```bash
# set by a choice
grep -o 'setsFlag: *"[a-z_]*"' story-data.js | sed 's/.*"\(.*\)"/\1/' | sort -u
# actually read
grep -o 's\.flags\.[a-z_]*' story-data.js | sed 's/s\.flags\.//' | sort -u
grep -o 'requiresFlag: *"[a-z_]*"' story-data.js | sed 's/.*"\(.*\)"/\1/' | sort -u
```

Anything in the first list but neither of the others is dead weight. It was 10
at the start of the fourth session; it is 8 now.

### Driving the real game

Chromium is preinstalled in cloud sessions at
`/opt/pw-browsers/chromium-1194/chrome-linux/chrome`. Use `playwright-core`,
and **do not commit it** — `npm install playwright-core` writes it into
`package.json`, so `git checkout -- package.json package-lock.json` afterwards.

Setup flow to automate: `Begin Your Journey` → fill the text input → `Continue`
→ click a class card. Note that a **non-Fighter who fights the bandits arrives
wounded**, landing on `keep_gate_wounded`, which has no "Rush the gate" option.
To reach the normal gate as a Cleric, take the "double the price" road.

**Two console errors are expected and are not regressions** (confirmed by
stashing all changes and re-running):
- `net::ERR_CONNECTION_RESET` — Google Fonts, unreachable in the sandbox. The
  game falls back to system serifs by design.
- `Failed to register a ServiceWorker … origin ('null')` — service workers
  require http(s); this always fails when opening the file over `file://`.

---

## Arcadia OS (separate repo — `tech-with-anthony/arcadia-os`)

Not part of this project; recorded here only because it was built in this repo
and the handoff matters.

A Debian **live-build** project that produces a bootable Debian 13 (trixie)
live+installer ISO for gaming: KDE Plasma, Steam, Lutris, Wine/Proton,
GameMode, MangoHud, Gamescope, RetroArch/Dolphin, OBS, Discord/Heroic/Bottles
via Flatpak, plus i386 multiarch and kernel tuning (notably
`vm.max_map_count = 2147483642`, which is what stops many Proton titles
crashing).

> **Status: unverified.** All 10 shell scripts pass `sh -n`/`bash -n`, but
> **`./build.sh` has never been run.** Package availability, Flathub reachability
> during the chroot phase, and boot behaviour are all untested. Building needs a
> Debian/Ubuntu amd64 host with root and loop devices — it cannot run in the
> cloud session. That is the next real milestone.

Design notes worth preserving:
- Only rock-solid packages go in `config/package-lists/`; anything that might be
  renamed between Debian releases lives in a guarded hook that skips rather than
  fails.
- The adaptive-icon and executable-bit lesson below applies to that repo too:
  **all 10 scripts must stay mode `100755`.** Pushing via the GitHub API's file
  endpoints flattens them to `100644`, producing a repo that looks fine and
  where `build.sh` and every chroot hook silently fail. Transfer via git
  (clone/bundle/push), never via the API.
...
├── story-data.js              # All 70 scene definitions (window.STORY)
├── adventure_standalone.html  # Single-file build, all JS inlined (~84KB)
├── manifest.json              # PWA manifest
├── sw.js                      # Service worker (offline caching)
├── icon-1024.png              # Source art; 512 and 192 are downscales
├── icon-512.png
├── icon-192.png
├── capacitor.config.json      # Capacitor config
├── package.json               # npm scripts: build (stage to www/), sync
├── android/                   # Capacitor Android platform (launcher icons in
│                              #   app/src/main/res/mipmap-*)
├── CLAUDE.md                  # This file
└── README.md                  # Player-facing description + developer notes
```

`adventure_standalone.html` is a **generated artefact**, not a source file. It
is not rebuilt automatically, and it was found stale in the fourth session —
after changing `engine.js` or `story-data.js` you must regenerate it or it will
silently ship the old story:

```python
html   = open("adventure.html").read()
story  = open("story-data.js").read()
engine = open("engine.js").read()
out = (html
  .replace('<script src="story-data.js"></script>', "<script>\n"+story.rstrip()+"\n</script>")
  .replace('<script src="engine.js"></script>',     "<script>\n"+engine.rstrip()+"\n</script>")
  .replace("<script>if ('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js');</script>", ""))
open("adventure_standalone.html","w").write(out)
```

The service-worker registration is deliberately stripped: a single-file build
opened over `file://` can never register one, so it only produces a console
error.
